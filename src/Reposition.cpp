...
void Reposition::kmeanPenalty(vector<Wifi*>& wifiSet, vector<Client*>& clientSet)
{
	size_t clusterCount = wifiSet.size();
	...
	while(true)
	{
		...
		for (size_t i = 0; i < clientSet.size(); i++)
		{
			...
			{
				Wifi* wifi = wifiSet[j];
				Real dist = Position::distance(*client, *wifi) + 10 * wifi->getClientCount();
...
void Reposition::kmeanRandom(vector<Wifi*>& wifiSet, vector<Client*>& clientSet)
{
	size_t clusterCount = wifiSet.size();
...
void Reposition::gravityPenalty(vector<Wifi*>& wifiSet, vector<Client*>& clientSet)
{
	vector<Vec2d> forces;
	vector<int> counts;
